...
class PlayList(Channel):
    ...
    @property
    def total_duration(self):   #возвращает объект класса datetime.timedelta с суммарной длительность плейлиста
        ...
        for video in self.get_service_playlist()['items']:
            ...
            try:
                ...
                sum_duration = sum_duration + delta
            except ValueError:
    ...
    @property
    def show_best_video(self):      #возвращает ссылку на самое популярное видео из плейлиста (по количеству лайков)
        max_likes = 0
        for video in self.get_service_playlist()["items"]:
